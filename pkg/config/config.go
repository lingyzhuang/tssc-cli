...
func (c *Config) EnableDisableProduct(name string, enabled bool, spec Product) error {
	spec.Enabled = enabled
	return c.SetProduct(name, spec)
}

func (c *Config) SetProductNamespace(name string, namespace string, spec Product) error {
	spec.Namespace = &namespace
	return c.SetProduct(name, spec)
}

func (c *Config) SetProductProperties(name string, properties map[string]interface{}, spec Product) error {
	if spec.Properties == nil {
		spec.Properties = make(map[string]interface{})
	}
	if err := mergo.Merge(&spec.Properties, properties, mergo.WithOverride); err != nil {
		return fmt.Errorf("failed to merge properties for product %s: %w", name, err)
	}
	return c.SetProduct(name, spec)
}
